...
class Backend extends Controller
{
    ...
    public function _initialize()
    {
        ...
        if (!$this->auth->match($this->noNeedLogin))
        {
            ...
            // 判断是否需要验证权限
            if (!$this->auth->match($this->noNeedRight)&& $path!='sms/add')
            {
                // 判断控制器和方法判断是否有对应权限
            ...
            $orginalurl = $this->request->url();
            if($this->auth->isSms($path)&&!$this->auth->isSuperAdmin()&&strstr($refer,'admin/sms/add')===false&&$this->request->isGet()){
                //发送短信
                $verfyCode = new VerifyCode();
                $result = $verfyCode->getAdminTelCode();
                $this->error('需要短信验证',url('sms/add'),'',0,['referer'=>$orginalurl]);
            }
        }
    ...
    public function result_return($result){
        if ($result) {
            $this->success('成功');
        } else {
            $this->error('失败');
