...
class Ajax extends Backend
{
    ...
    public function excel(){
        ...
        $result = $riderModel->importExcel($path);

        if($result){
            $this->success('导入成功', null,['code'=>100000]);
        }else{
            $this->success('导入失败', null,['code'=>100001]);
        }
    ...
    public function package(){
        ...
        $errstr = '';
        if(!empty($this->request->get('row.start_time'))){
            //开始时间必须为周一并且大于等于当前时间
            $starttime = strtotime($this->request->get('row.start_time'));
            if($starttime < strtotime('Y-m-d',time())){
                $errstr = '开始时间不能是过去时间';
                Session::set('starttime','');
                die($errstr);

            }

            if(date('w',strtotime($this->request->get('row.start_time')))!=1){
                $errstr = '开始时间不周一';
                Session::set('starttime','');
                die($errstr);
            }
            ...
            //缓存开始时间
            Session::set('starttime',$this->request->get('row.start_time'));
        }

        if(!empty($endtime = $this->request->get('row.end_time'))){
            //结束时间
            ...
            if(date('Y-m-d',strtotime('+6day',strtotime($starttime)))!=$endtime){
                $errstr = '结束时间与开始时间不匹配';
                die($errstr);
            }
    ...
    public function qiniu(){
        ...
        if($key = $qiniu->upload($uploadDir.$fileName)){
            ...
            if(empty($action)){
                ...
                $attachment = model("attachment");
                $aid = $attachment->create(array_filter($params));
                \think\Hook::listen("upload_after", $attachment);
            }else{
                $params = array(
                    'url' =>$key,
                ...
                $aid['id'] = Db::table('tb_attachment')->insert($params);
            }


            $this->success('上传成功', $domain, [
                'url' => 'http://'.$domain.'/'.$key,
                'id'  => $aid['id'],
            ]);
        }else{
