    ...
    public function notify(){
        ...
        if($flag){
            ...
            if($_POST['trade_status'] == 'TRADE_SUCCESS'){
                $order_pay = new PayRecordDao();
                $order_status = $order_pay->get_by_payno($_POST['out_trade_no']);

                if($order_status['status'] == 1){
                    $order = new Tborder();
                    $result = $order->set_order($_POST['out_trade_no']);
                    if($result['msg'] == "success"){
                        $tb_order_dao = new TborderDao();
                        $order_info = $tb_order_dao->get_by_payno_all($_POST['out_trade_no']);
                        $this->order_back($order_info);
                       die('SUCCESS');
                    }
                }
        ...
        if($payid){
            ...
            if($flag){
                ...
                if($_POST['trade_status'] == 'TRADE_SUCCESS'){
                    ...
                    if($order_status['status'] == 1){
                        $order = new Tborder();
                        $result = $order->set_tuan_order($_POST['out_trade_no']);
                        if($result['msg'] == "success"){
                            $tb_order_dao = new TborderDao();
                            $order_info = $tb_order_dao->get_by_payno_all($_POST['out_trade_no']);
                            $this->order_back($order_info);
