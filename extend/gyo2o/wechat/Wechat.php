...
class Wechat
{
	...
	{
	    $options = array(
	        			'token'=>config('wechat.WE_CHAT_TOKEN'), //填写你设定的key
	        			'appid'=>config('wechat.WE_CHAT_APPID'), //填写高级调用功能的app id
	        			'appsecret'=>config('wechat.WE_CHAT_APPSRCRET'), //填写高级调用功能的密钥
	        		);
		...
		//读取 参数配置
		$this->token = config('wechat.WE_CHAT_TOKEN');
	...
	private function checkSignature()
	{
		...
		if( !empty($tmpStr) && !empty($signature) ){
			return true;
		}else{
			return false;
		}
	...
	public function getRev()
	{
		...
		//$this->write_log('log.txt',$postStr);
		$this->log($postStr);
		if (!empty($postStr)) {
	 ...
	 * 错误信息打印成txt文件
	 */
	public function write_log($txtName, $log) {
	    // 这里是你记录调试信息的地方 请自行完善 以便中间调试
	    // 写入文件到txt
	    $open = fopen ( $txtName, "a+" ); // 'a' 写入方式打开，将文件指针指向文件末尾。如果文件不存在则尝试创建之。'a+' 读写方式打开，将文件指针指向文件末尾。如果文件不存在则尝试创建之。
	    fwrite ( $open, $log );
	    fclose ( $open );
	}
	/**
